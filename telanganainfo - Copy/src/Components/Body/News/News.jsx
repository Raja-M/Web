      ...
      () => { 
        ...
        if ( isLoading ){
            ...
            return(   
              ...
              <Link    to={`${newsItem.id}`} style={{ textDecoration: 'none'  }}  > 
                    <article   style={{ display:'block'  }}>
                    <h3>{  newsItem.title + " " + newsItem.id}</h3> 
                            <Author userId={newsItem.userId} > </Author>
                            
                    <p> {newsItem.content?.substring(0,100)}</p>
                    </article>
              </Link> 
              <ReactionButton key={newsItem.id} newsItem={newsItem}> </ReactionButton>
              { Detail ? (  <NavLink to={`/manage/news/${newsItem.id}` } >
